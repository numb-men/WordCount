...
public class WordCount {
    ...
    public byte[] filterAscii(byte[] bytes) {
        ...
        for (byte aByte : bytes) {
            if (aByte < 128 && aByte >= 0) {
                noAsciiBytesTemp[j++] = aByte;
            }
        }
    ...
    public void init(String[] args) {
        ...
        // 长度：字节数组长度
        int bytesLength = bytes.length;
        // 取初始HashMap大小为字节长度的1/5
        wordMap = new HashMap<>(bytesLength / 5);
    }
    ...
    public static void main(String[] args) {
        ...
        long preTestTime = System.currentTimeMillis();
        WordCount wordCount = new WordCount();
        wordCount.init(args);
        wordCount.count();
        wordCount.writeResult();
        long afterTestTime = System.currentTimeMillis();
        System.out.println("用时：" + (afterTestTime - preTestTime) + "ms");
    }
}
